...
Every [dimension](/concepts/trust-score/introduction) is a pre-configured Harness. In addition, each [Scenario](/concepts/evaluation-components/scenario) is also a Harness. You can run an evaluation including one or more pre-defined Harnesses through either the UI or the Python client.
