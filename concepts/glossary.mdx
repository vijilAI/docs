...
**Engagement**
The object created by a Red Team run. An Engagement runs in Waves, generating Attack Seeds, attacking the Agent over multiple turns, judging transcripts, and reflecting to plan the next Wave.
...
**Judgment**
A judge's verdict on one Red Team attack transcript. A Judgment records whether harmful content was delivered (FULL, PARTIAL, or NONE) and whether the harm is verified or needs owner review.
...
**Red Team**
A Diamond mode that runs adversarial attacks against a registered Agent to uncover vulnerabilities, policy violations, leaked artifacts, and successful attack strategies. A Red Team run creates an Engagement and attacks in Waves. In the Console it is the Adaptive tab. Red Team is also available from the CLI, MCP, and REST API.

---

**Reflection**
A Red Team Engagement's summary of one Wave, recording which strategies worked and which risks remain uncovered. Reflections are what make later Waves more targeted than earlier ones.
...
**Wave**
One iteration of a Red Team Engagement. A wave generates seeds, runs attackers, judges transcripts, and produces reflections that guide the next wave.
