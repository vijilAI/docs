...
**Red Team**
A Diamond mode that runs adversarial attacks against a registered Agent to uncover vulnerabilities, policy violations, leaked artifacts, and successful attack strategies. A Red Team run creates an Engagement and attacks in Waves. In the Console it is the Adaptive tab; from the CLI and MCP it is the xteaming command group.
