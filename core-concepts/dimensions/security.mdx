...
Vijils tests for the vulnerability of LLMs to popular types of attacks observed with production deployments.
...
This test is designed to disrupt a system prompt by appending an adversarial suffix. To implement these tests, Vijil follows the approach of [Zou _et al_, 2023](https://arxiv.org/abs/2307.15043) that automatically produces adversarial suffixes by a combination of greedy and gradient-based search techniques.
...
that is an indication of the agent not checking output for malicious or dangerous content.
...
Vijil follows the [PromptInject](https://openreview.net/forum?id=qiaRo_7Zmug) framework.
