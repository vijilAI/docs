...
Vijil evaluates the robustness of an LLM using one module of 30 tests.
...
Vijil adapted the prompts in AdvGLUE to our purposes, keeping only those perturbed input prompts that truly preserve the meanings of the original prompts. Taking all combinations of tasks and attacks, Vijil probes whether the model's performance declines on a task when the input prompts are perturbed by an attack vector.
