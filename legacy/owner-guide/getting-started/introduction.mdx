...
AI agents stall between development and production. Security wants proof the agent will not leak data. Compliance wants proof it follows policy. The business wants proof it actually works. Without objective evidence, these conversations become negotiations based on intuition, and agents sit in pilot programs indefinitely.
...
<Info>
If you are a developer integrating Vijil into your codebase, see the [Developer Guide](/developer-guide/getting-started/introduction). This guide focuses on evaluation workflows through the Vijil console.
</Info>
...
<CardGroup cols={2}>
  <Card title="Business Owner" icon="rocket">
    ...
    **Key Tasks:**
    - <Icon icon="square-check" /> [Run Evaluations](/owner-guide/run-evaluations/running-evaluations) — Test efficiently
    - <Icon icon="square-check" /> [Understand Results](/owner-guide/run-evaluations/understanding-results) — Know when you're ready
    - <Icon icon="square-check" /> [Deliver Trust Reports](/tutorials/evaluate-agents/reports) — Evidence for stakeholders
  </Card>
  <Card title="Risk Officer" icon="shield">
    ...
    **Key Tasks:**
    - <Icon icon="square-check" /> [Set Policies](/owner-guide/simulate-environment/policies) — Testable requirements
    - <Icon icon="square-check" /> [Review Trust Reports](/tutorials/evaluate-agents/reports) — Defensible audit artifacts
    - <Icon icon="square-check" /> [Audit Agent Behavior](/owner-guide/protect-in-production/observability) — Continuous risk visibility
  </Card>
</CardGroup>
...
Agents fail in ways that do not look like bugs. They hallucinate facts, comply with requests they should refuse, and behave differently under adversarial pressure than in demos. Vijil evaluates agents across three dimensions that capture these failure modes:
...
| **Safety** | Does the agent stay within acceptable boundaries? | [Policy](/concepts/glossary#vijil) violations, harmful content, unauthorized actions |

Each evaluation produces a **Trust Score**—a quantitative measure of where the agent is strong, where it is vulnerable, and how it compares to alternatives.
...
<Steps>
  ...
  <Step title="Register your agent and its environment">
    Add your agent by providing as little information as only it is URL and description or as much information as its source code. Vijil uses the behavior and composition of the agent to test and defend it.
  </Step>
...
<CardGroup cols={2}>
  ...
  <Card title="First Evaluation" icon="play" href="/tutorials/evaluate-agents/setup-evaluate">
