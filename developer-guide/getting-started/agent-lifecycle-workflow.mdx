...
- Convert successful Red Team attacks into regression tests.
...
<Tabs>
  <Tab title="Console">
    ...
    9. If the deployment includes Darwin, review its proposals before accepting them.
  </Tab>
  <Tab title="SDK">
...
<Tabs>
  <Tab title="Console">
    ...
    3. Replay the relevant Red Team attacks.
  ...
  <Tab title="SDK">
    1. Run `client.evaluate(agent_id, baseline=True)`.
    2. Use `harness_id` to run targeted Harnesses when needed.
    3. Replay relevant adaptive attacks with `client.xteam()`.
    4. Compare the returned scores and findings with the saved baseline.
    5. Check staging traces with `client.monitor.traces()`.
  </Tab>
</Tabs>
