...
| **Evaluate** | Establish a [Trust Score](/concepts/trust-score/introduction) baseline | Trust Score and Trust Report |
| **Harden** | Add and test [runtime Guardrails](/developer-guide/protect/overview) | Applied Dome configuration and staging telemetry |
| **Improve** | Find and address deeper weaknesses | [Red Team findings](/owner-guide/run-evaluations/understanding-results#understanding-red-team-results) and reviewed changes |
...
A [Diamond Evaluation](/developer-guide/evaluate/overview) measures the Agent against known standards. A [Red Team campaign](/owner-guide/run-evaluations/running-evaluations#running-a-red-team-campaign) searches for weaknesses that fixed tests might miss. Start with a baseline Trust Score Evaluation across Reliability, Security, and Safety.
...
Use the [Evaluation findings](/developer-guide/evaluate/understanding-results#reading-findings) to choose [Dome runtime protection](/developer-guide/protect/overview):
...
Start with an [adaptive Red Team campaign](/owner-guide/run-evaluations/running-evaluations#running-a-red-team-campaign) against the hardened Agent. Red Teaming investigates tool misuse, data leakage, policy violations, and weaknesses that fixed Evaluation cases might not expose. Use the findings to decide what to change.
...
<Tabs>
  ...
  <Tab title="SDK">
    ...
    5. Where Darwin is available, run [`client.adapt()`](/developer-guide/sdk/lifecycle-methods#client-adapt).
    6. Review the generated proposals with [`client.proposals`](/developer-guide/sdk/resources#client-proposals).
