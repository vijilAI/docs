...
description: 'Start, monitor, and retrieve Diamond Evaluations programmatically using the CLI, MCP, REST API, or Python SDK.'
---
...
<Tabs>
  ...
  Pass either `baseline=True` or a `harness_id`. To start without blocking, call `client.evaluations.create(agent_id="<agent-id>", baseline=True)` and poll `client.evaluations.show()` yourself.
