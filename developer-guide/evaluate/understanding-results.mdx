...
description: 'Interpret Diamond Evaluation findings using the Dimensions of Trust framework.'
---

<Tip>
**TL;DR:** Behavioral evaluation results include a [Trust Score](/concepts/trust-score/introduction) (0–100), per-dimension breakdowns, severity-rated findings, and remediation guidance. A score at or above 70 passes the deployment threshold. Red Team evaluations produce [attack evidence and a report](#red-team-results) instead of a Trust Score.
</Tip>

Behavioral evaluation results reveal how your [Agent](/owner-guide/register-agents/what-is-an-agent) behaves across the three pillars of trustworthy AI: [Reliability](/concepts/trust-score/reliability), [Security](/concepts/trust-score/security), and [Safety](/concepts/trust-score/safety). This page explains how to read the Trust Score and prioritize remediation. For Red team evaluation findings, see [Red Team results](#red-team-results).
...
Each finding in a behavioral evaluation report includes:
...
Each behavioral evaluation produces a Trust Report, a structured PDF that moves from a high-level verdict down to individual Probe results and actionable remediation guidance. You can download a [sample report](/assets/vijil-console-eval-report.pdf) to follow along. The report has six sections.
...
## Comparing Behavioral Evaluations

Run behavioral evaluations before and after changes to track improvement:
...
A Red Team evaluation produces attack evidence and a report, not a Trust Score. Use its `evaluation_id` to inspect the run and connect findings to the attacks that exposed them:

| Evidence | What It Shows |
|----------|---------------|
| **Status** | Current phase, wave, attacker progress, and any failure or degradation details |
| **Waves and seeds** | The attack goals explored in each wave |
| **Attacks and judgments** | Transcripts, attempted strategies, and assessments of Agent behavior |
| **Reflections** | What worked and how later waves adjusted |
| **Report** | Vulnerabilities and successful attack strategies across the run |

See [Red Team in Diamond](/concepts/platform/diamond#red-team-in-diamond) for how these waves produce the final findings.
