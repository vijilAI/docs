...
<Note>
  `DOME_INFERENCE_URL` configures the remote Detectors used to score a local Diamond Evaluation. This variable differs from the in-process Dome Guardrails that you enable later in the tutorial.
</Note>
...
- A Vijil API key, Console gateway URL, and team ID
...
python -m pip install --upgrade vijil-sdk "vijil-dome[pii]"
```

Upgrading `vijil-sdk` and `vijil-dome[pii]` after installing the sample requirements ensures that you use the current local Evaluation CLI and install the optional Presidio dependencies required by the fast Dome configuration.
...
Export your Vijil credentials and deployment values:

```bash
export VIJIL_API_KEY="<vijil-api-key>"
...
The CLI reads the gateway and default team from `~/.vijil/config.toml`. `VIJIL_API_KEY` authenticates both the Console requests and the remote Detector requests. The Evaluation runner appends `/v1/detect` to `DOME_INFERENCE_URL`, so set it to the gateway base URL rather than the full detection route.
...
The Agent list verifies that the CLI can authenticate with the configured gateway. You can also run `vijil auth login` to store your API key. Keep `VIJIL_API_KEY` exported for this tutorial because the local Detector dispatcher reads it directly.

## Run the Unprotected Agent

Start the Travel Agent without Dome:
...
In a second terminal, activate the same virtual environment and send a smoke-test request:
...
export VIJIL_AGENT_ID="$AGENT_ID"
export TEAM_ID="<team-id>"

python agent.py
```

The Travel Agent loads `dome_config_fast.toml` and creates a `DomeHookProvider` for every Strands Agent instance. The provider runs a Guard on the last user message before each model call and on the model response after each call. When an enforced Guardrail flags content, the hook replaces it with a blocked message.
...
Do not create a tunnel as a workaround. The current local architecture does not require or use ngrok, and this guide does not expose the intentionally vulnerable Travel Agent to a public URL.

### Darwin Is in Private Preview

[Darwin](/concepts/platform/darwin) is private preview. This tutorial does not run an evolution, apply a proposal, or adapt the Agent.
...
| Registration is laptop-only or Console calls return authentication errors | Export a valid `VIJIL_API_KEY`, verify `gateway.url`, and register the Agent again. |
