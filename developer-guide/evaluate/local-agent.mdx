...
python -m pip install vijil-sdk fastapi uvicorn pyngrok httpx
```
...
RESTRICTED_AGENT_NAME = "vijil-market-researcher"
...
def load_market_researcher_prompt(plugin_dir: Path) -> str:
    agent_file = plugin_dir / "agents" / "market-researcher.md"
    try:
        source = agent_file.read_text(encoding="utf-8")
    except OSError as exc:
        raise RuntimeError(f"Could not read {agent_file}") from exc

    sections = source.split("---", maxsplit=2)
    if len(sections) != 3 or not sections[2].strip():
        raise RuntimeError(f"Could not parse the Agent prompt in {agent_file}")
    return sections[2].strip()


def restricted_agent_definition(plugin_dir: Path) -> dict[str, dict[str, object]]:
    upstream_prompt = load_market_researcher_prompt(plugin_dir)
    restricted_prompt = f"""{upstream_prompt}
    return {
        RESTRICTED_AGENT_NAME: {
            "description": "A text-only Market Researcher for local Evaluation",
...
def run_market_researcher(prompt: str) -> str:
    ...
    agent_definition = restricted_agent_definition(plugin_dir)
    command = [
        "claude",
        "--bare",
        "--plugin-dir",
...
def main() -> None:
    ...
    parser.add_argument(
        "--smoke-test",
        action="store_true",
        help="Run one Agent request without starting the proxy or tunnel",
    )
    args = parser.parse_args()

    if args.smoke_test:
        result = run_market_researcher(
            "Give a short overview of the semiconductor equipment market. "
            "Mark every unsupported figure as [UNSOURCED]."
        )
...
Run the same restricted Agent invocation that the proxy will use:

```bash
python local_agent_evaluation.py --smoke-test
```

Continue only after the command returns JSON with a non-empty `result` field. Because the command uses `--bare`, `--tools ""`, and `--strict-mcp-config`, the Agent has no runtime file, shell, Skill, or MCP tools.
