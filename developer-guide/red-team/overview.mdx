...
| **CLI** | `vijil xteaming`, 15 commands. See the [Red Team CLI Reference](/developer-guide/cli/red-team#red-team-engagements). |
| **MCP** | The `xteaming_*` tools, 16 of them. See the [MCP tool catalog](/developer-guide/agentic/tools#red-team-engagements). |
| **REST API** | The Engagements endpoints under `/v1/xteaming/engagements`. |
...
<Tabs>
  ...
  <Tab title="REST API">
    ```bash
    curl -X POST "$VIJIL_API_URL/v1/xteaming/engagements" \
...
| `xteaming` | How hard each attacker works before giving up |
| `target_resilience` | Rate limiting, circuit breaker, and a pre-wave reachability check, for flaky or capacity-limited targets |

<Note>
The exact override keys come from the `vijil-redteam` schemas package rather than from the Console, so run `vijil xteaming create --help` to see the set your deployment accepts.
</Note>
...
<Tabs>
  ...
  <Tab title="REST API">
    ```bash
    curl "$VIJIL_API_URL/v1/xteaming/engagements/$ENGAGEMENT_ID?team_id=$TEAM_ID" \
      -H "Authorization: Bearer $VIJIL_TOKEN"
    ```
  </Tab>
</Tabs>
...
|---------------|----------|
| Attack seeds | `GET /v1/xteaming/engagements/{id}/seeds` |
| Attacker runs | `GET /v1/xteaming/engagements/{id}/attacks` |
| One attacker run | `GET /v1/xteaming/engagements/{id}/attacks/{wave_id}/{seed_id}` |
| Judgments | `GET /v1/xteaming/engagements/{id}/judgments` |
| One judgment | `GET /v1/xteaming/engagements/{id}/judgments/{wave_id}/{seed_id}` |
| Reflections | `GET /v1/xteaming/engagements/{id}/reflections[/{wave_id}]` |
| Final report | `GET /v1/xteaming/engagements/{id}/report` |
| Report as Markdown or PDF | `GET /v1/xteaming/engagements/{id}/report.md` or `report.pdf` |
