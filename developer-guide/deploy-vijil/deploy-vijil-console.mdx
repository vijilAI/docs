...
> **Already have a cluster and AWS resources?** Jump to [Phase 2: IAM and EKS Add-ons](#phase-2-iam-and-eks-add-ons) and then [Phase 3: Helm Values and Secrets](#phase-3-helm-values-and-secrets).
...
<img src="/images/developer-guide/deploy-vijil/Deploying Vijil Console Architecture Overview.png" alt="Vijil Console Architecture showing VPC, subnets, NLB, EKS, and RDS" style={{maxWidth: '700px', margin: '2rem auto', display: 'block'}} />

All application workloads and the database live in **private subnets**. Only a single nginx NLB sits in a **public subnet**, both `console.*` and `console-api.*` resolve to this one NLB, and the unified nginx router handles splitting frontend vs API traffic internally. EKS nodes reach S3 and ECR via NAT Gateway.
...
## Phase 2: IAM and EKS Add-ons

### 2.1 IAM Policy for S3
...
### 2.2 S3 Access for Pods

There are two supported approaches. **EKS Pod Identity (Option A)** and **Node IAM Role (Option B)**. EKS Pod Identity is recommended, it's what the current dev environment uses and scopes credentials to specific service accounts rather than all pods on a node.
...
Build and push images to your own registry, then override `*.image.repository` in your values file (see [Phase 3](#phase-3-helm-values-and-secrets)).

### 2.5 Bedrock AgentCore (Diamond / custom harness)
...
### 2.6 Staging / Non-Dev: Custom Harness AgentCore Runtime

In dev, a custom harness AgentCore runtime already exists in the account and is looked up by name. In any non-dev account (staging, customer), that runtime doesn't exist — you need to create one and point the Console at it via `CUSTOM_HARNESS_AGENT_RUNTIME_ARN`.
...
### 2.7 Diamond Artifacts S3 Access (staging / customer accounts)

The Diamond evaluation engine downloads detector configs from a `vijil-artifacts-bucket` that lives in Vijil's dev account. In any non-dev account, your EKS nodes need cross-account read access to that bucket.
...
## Phase 3: Helm Values and Secrets
...
secrets:
  vijil-console:
    POSTGRES_PASSWORD: "the-rds-password-you-set-in-phase-1.3"
...
```yaml
commonEnv:
  # Database (RDS endpoint from Phase 1.3)
  ...
  service:
    aws:
      nlb:
        ...
        subnets: "subnet-public-1-id,subnet-public-2-id"  # public subnet IDs from Phase 1.1
      tls:
        enabled: true
...
## Phase 4.5: Deploy Darwin (Evolution Engine)
