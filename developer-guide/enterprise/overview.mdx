...
This section covers self-hosted deployment. If you are evaluating whether self-hosting is right for you, start with the qualifying questions below.
...
<Note>
  If your primary concern is data privacy, note that the cloud platform does not store your agent's responses—only evaluation metadata. Contact [contact@vijil.ai](mailto:contact@vijil.ai) to discuss your specific requirements before committing to self-hosting.
</Note>
...
<img
  src="/images/developer-guide/enterprise/overview/enterprise-architecture.svg"
  alt="Vijil system architecture showing Diamond, Dome, and Console components"
  style={{maxWidth: '700px', margin: '2rem auto', display: 'block'}}
/>
...
| **Database Admin** | Provision PostgreSQL, configure backups |
...
A typical self-hosted deployment invovles:

1. **Prerequisites** — Provision infrastructure, configure networking
2. **Deployment** — Install Vijil components, configure data stores
3. **Integration** — Connect Auth0, test agent connectivity
4. **Validation** — Run test evaluations, verify Guardrail functionality
...
<CardGroup cols={2}>
  ...
  <Card title="Contact Sales" icon="phone-outgoing" href="mailto:contact@vijil.ai">
