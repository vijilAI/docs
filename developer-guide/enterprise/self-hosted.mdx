...
Vijil deploys as a set of Helm charts to your Kubernetes cluster. The deployment connects to your provisioned data stores (PostgreSQL, S3).
...
See the [deployment guide](/developer-guide/deploy-vijil/deploy-vijil-console) for all available Helm configuration options.
...
Navigate to `https://vijil.your-company.com` and verify you can log in.
...
- Check that the console is accessible and responding
...
Review the [release notes](/legacy/references/release-notes) before upgrading for any breaking changes.

## Next Steps

<CardGroup cols={2}>
  <Card title="Deployment Guide" icon="server" href="/developer-guide/deploy-vijil/deploy-vijil-console">
    Full deployment walkthrough with Helm values
  </Card>
