...
Once you have your M2M credentials stored as CI/CD secrets, use them to authenticate the Vijil client:

```python
import os
from vijil import Vijil

def get_vijil_client():
    """Create authenticated Vijil client using M2M credentials."""
    return Vijil(
...
client = get_vijil_client()
```

<Note>
If the exact authentication flow differs for your deployment, contact [support@vijil.ai](mailto:support@vijil.ai) for the correct M2M setup steps for your account.
</Note>
...
def get_token():
    """Get authenticated Vijil client from M2M credentials."""
    return Vijil(
        client_id=os.environ["M2M_CLIENT_ID"],
        client_secret=os.environ["M2M_CLIENT_SECRET"],
        client_token=os.environ["M2M_CLIENT_TOKEN"]
...
<CardGroup cols={2}>
  ...
  <Card title="Testing Strategies" icon="settings" href="/developer-guide/cicd/testing-strategies">
