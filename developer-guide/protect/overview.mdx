...
For tool-using Agents, [Trust Runtime](/developer-guide/protect/trust-runtime) extends Dome with workload identity, Console or local constraints, tool-level mandatory access control, signed tool manifests, tool attestation, and structured audit events. Use `secure_agent()` with LangGraph, Google ADK, or Strands. Tool enforcement depends on each framework's integration points; LangGraph tool wrapping requires discoverable tool functions on the compiled graph.

## Choose a Protection Interface

| Interface | Use It When | Protection |
|-----------|-------------|------------|
| `Dome` | You need content Guardrails for inputs and outputs | Guards and Detectors |
| `secure_agent()` | You use a supported agent framework | Content Guards, identity, framework-dependent tool access control, attestation, and audit events |
| `TrustRuntime` | You need direct or custom integration | Individual methods for each trust enforcement point |
...
|--------|-----------|----------|
| **Registered Agent** | Pull from your Console Agent definition | Production: keeps configuration in sync with your policy |
...
Dome integrates with popular frameworks and runtime environments. See the framework developer guides for available integration patterns.
