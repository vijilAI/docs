...
description: 'Detailed list of every built-in Detector, grouped by category. Each entry
...
[Detectors](/concepts/evaluation-components/detector) that identify adversarial inputs such as prompt injections,
...
completions endpoint. Defaults to [GPT-OSS-Safeguard-20B](https://huggingface.co/openai/gpt-oss-safeguard-20b) on Groq
...
predictions are escalated to [GPT-OSS-Safeguard-20B](https://huggingface.co/openai/gpt-oss-safeguard-20b). ~5ms average latency,
...
LLM-based security classification via [LiteLLM](https://litellm.ai/).
