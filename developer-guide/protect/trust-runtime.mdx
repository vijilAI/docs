...
| `secure_agent()` | You use LangGraph, Google ADK, or Strands | Identity, content Guards, tool access control, attestation, and audit events |
...
The Controls Engine is a separate policy-as-data interface. Use Trust Runtime when you need identity-bound enforcement and framework integration in addition to content checks.
...
Pass your agent object and its registered `agent_id` to `secure_agent()`. Start in `warn` mode while you validate the integration:
...
Use the returned object instead of calling `graph.compile()` separately.
...
Verify the manifest signature against the Console public key before you pass the manifest to `TrustRuntime`. The runtime loads the manifest and uses it for endpoint attestation; `attest()` does not verify the manifest signature.
...
<Note>
Create and sign manifests through the Console workflow. A dedicated Manifest CLI reference is forthcoming; this page does not prescribe an unconfirmed packaged command.
</Note>
...
Events contain an `event_type`, `agent_id`, timestamp, and event-specific attributes. The runtime emits events for content Guard decisions, tool access checks, attestation results, identity or mTLS downgrade conditions, disabled Guards, and enforcement-mode downgrade attempts.
