...
Red Team tools manage [Personas](/owner-guide/simulate-environment/personas), [Policies](/owner-guide/simulate-environment/policies), and adversarial [Engagements](/concepts/red-team/engagements).
...
An Engagement runs adversarial attacks in waves, learning from each wave to plan the next. See [Engagements](/concepts/red-team/engagements) for the mechanics.

| Tool | Description |
|------|-------------|
| `xteaming_create` | Create an Engagement |
| `xteaming_list` | List Engagements |
| `xteaming_status` | Get Engagement status |
| `xteaming_results` | Get Engagement results |
| `xteaming_cancel` | Cancel a running Engagement |
| `xteaming_delete` | Delete an Engagement |
| `xteaming_seeds` | List attack seeds |
| `xteaming_attacks` | List attacker runs |
| `xteaming_attack` | Get one attacker run |
| `xteaming_judgments` | List judgments |
| `xteaming_judgment` | Get one judgment |
| `xteaming_reflections` | List reflections |
| `xteaming_reflection` | Get one wave's reflection |
| `xteaming_report` | Get the final report |
| `xteaming_report_md` | Get the final report as Markdown |
| `xteaming_report_pdf` | Get the final report as PDF |

### `xteaming_create`

Create and start an Engagement against a registered Agent.

| Parameter | Type | Description |
|-----------|------|-------------|
| `agent_id` | str | Agent to attack. Required. |
| `persona_ids` | list | Persona IDs to use |
| `policy_ids` | list | Policy IDs the judge tests against |
| `redteam` | dict | Wave and seed overrides |
| `xteaming` | dict | Attacker behavior overrides |
...
Get an Engagement's live state: phase, current wave, attackers completed, and attackers errored.

<Prompt description="What wave is the red team engagement on?">
What wave is the red team engagement on?
</Prompt>

### `xteaming_judgments`

List judgments for an Engagement, optionally filtered to one wave. Each judgment reports whether the attack succeeded and the potential harm.

<Prompt description="Show me every successful red team judgment and what was leaked">
Show me every successful red team judgment and what was leaked
</Prompt>

### `xteaming_report`

Get the final report, with clustered vulnerabilities, policy violations, leaked artifacts, and successful strategies. Use `xteaming_report_md` or `xteaming_report_pdf` for a shareable rendering.

<Prompt description="Summarize the red team report and tell me what to fix first">
Summarize the red team report and tell me what to fix first
</Prompt>
