...
```json
{
  ...
  "status": "failed",
  "error": "Cannot reach target agent at http://localhost:9000"
}
```
...
| Error | Cause |
|-------|-------|
| `Cannot reach target agent` | Target URL unreachable or not an A2A agent |
| `Harness not found` | Invalid harness_id or missing resources |
| `LLM authentication failed` | Invalid or missing API key |
...
Set global configuration for tools.
