---
title: "Integrating Vijil Evaluate into CI/CD"
description: ""
---
You can use Vijil Evaluate to automatically run evaluations as part of your CI/CD pipeline. To do that, you will need to obtain machine-to-machine secrets and use those to get and refresh authentication tokens. This is necessary because the regular user token expires every 24 hours.
...
Click **View long-lived token** to view the secrets. This option is available if they have not been viewed yet.
...
```env title=".env.example" icon="lock"
...
Now you can use `simple_access_token` to authenticate requests to the Vijil Evaluate API. Save it as `VIJIL_API_KEY` in your environment file and you can run the code samples in our [quickstart](/developer-guide/getting-started/quickstart).
