...
The `DOME_API_KEY` is an authentication key for validating incoming API requests. If you enable Guardrails that make external API calls such as OpenAI moderation or detectors hosted on Groq or any other AI inference service, include the corresponding API keys here as well.
...
This endpoint updates the default Dome configuration to include specific input and output Guards, as well as the methods used for each Guard.
...
Dome groups Guards into Bedrock policy categories based on the Guard type.
...
The Dome container is well set up for instrumentation to emit telemetry data that is metrics, traces, and logs using OpenTelemetry (OTEL). This allows you to monitor Dome's behavior and performance from any compatible observability backend.
