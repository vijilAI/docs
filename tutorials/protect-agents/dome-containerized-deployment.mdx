...
Pull the prebuilt [Dome container image](ghcr.io/vijilai/vijil-dome-marketplace) from GitHub Container Registry:
...
This endpoint updates the default Dome configuration to include specific input and output [Guards](/core-concepts/components/guard), as well as [the methods used for each Guard](/tutorials/protect-agents/detection-methods).
...
The `/guardrails/apply` endpoint evaluates content using Dome Guardrails and returns results in a structure compatible with the `ApplyGuardrail` API from [Amazon Bedrock](https://aws.amazon.com/bedrock/).
