---
title: "Use Dome as a Containerized Deployment"
...
The `DOME_API_KEY` is an authentication key for validating incoming API requests. If you enable [Guardrails](/concepts/defense/guardrail) that make external API calls such as OpenAI moderation or detectors hosted on Groq or any other AI inference service, include the corresponding API keys here as well.
...
This endpoint updates the default Dome configuration to include specific input and output [Guards](/concepts/defense/guard), as well as [the methods used for each Guard](/developer-guide/protect/detection-methods).
...
To check and [Guard](/concepts/defense/guard) against inputs to agents, send a `GET` request to the `/async_input_detection` endpoint with the input prompt. For example:
...
The `/guardrails/apply` endpoint evaluates content using [Dome Guardrails](/concepts/defense/guardrail) and returns results in a structure compatible with the `ApplyGuardrail` API from [Amazon Bedrock](https://aws.amazon.com/bedrock/).
...
Dome groups [Guards](/concepts/defense/guard) into Bedrock policy categories based on the Guard type.
