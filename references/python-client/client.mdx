...
Class for handling scenarios API requests.
...
List all scenarios. Returns list of dicts where each dict contains the metadata for a scenario or a pandas dataframe if format is "dataframe" as `List(dict) or pandas.DataFrame`.
...
List all probes. Returns list of dicts where each dict contains the metadata for a probe, or a pandas dataframe if format is "dataframe" as `List(dict) or pandas.DataFrame`.
...
- `format` (any): The format of the report ('html', 'pdf', 'md').
...
- `agent_id` (Optional[str], optional): The agent configuration ID to filter evaluations by, defaults to None.
...
- `timeout` (float, optional): The timeout in seconds for API calls during evaluation. Defaults to None
...
Lists all available detectors from the API. Returns the list of available detector ids as `list`.
